...
object DStreams {
  ...
  /*
    - define input sources by crating DStreams
    - define transformations on DStreams
    - call in action on DStreams
  ...
  def readFromSocket() = {
    ...
    // action
//    wordsStream.print()
    wordsStream.saveAsTextFiles("src/main/resources/data/words") // each folder = RDD = batch, each file = a partition RDD
