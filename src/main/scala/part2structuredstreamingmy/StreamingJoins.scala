...
object StreamingJoins {
  ...
  val guitaristsBands = guitarPlayers.join(bands, guitarPlayers.col("band") === bands.col("id"), "inner")
  ...
  def  joinStreamWithStream() = {
    ...
    val streamedJoin = streamedBandsDF
      .join(streamedGuitaristsDF, streamedGuitaristsDF.col("band") === streamedBandsDF.col("id"))
